...
void MainWindow::TimeData_Update(void)
{
    ...
    for (size_t i = 0; i < sizeof(buf) / sizeof(buf[0]); i++) {
        // debug
        buf[i] = ((i << 12) & 0xf000) | (int(2047*sin(f++ + i*0.1) + 2048) & 0xfff);
        Slide_Window::get_instance().add_data(buf[i]);
    }
    QVector<double> x, y;
    int i = 0;
    for(auto& data: Slide_Window::get_instance().get_seq()) {
        x.push_back(i++);
        y.push_back(data.data * max_voltage / 0xfff);
    }
    Show_Plot(customPlot, x, y);
    Slide_Window::get_instance().clear();
}

// 曲线更新绘图
void MainWindow::Show_Plot(QCustomPlot *customPlot, QVector<double> x, QVector<double> y)
{
    static double cnt;
